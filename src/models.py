...
class KMeans(nn.Module):
    def __init__(self, num_cluster, seed, hidden_size, input_size, output_size,gpu_id=0, device="cpu"):
        ...
        self.kan = KAN(
            layers_hidden=[2*hidden_size, 2*hidden_size, hidden_size],
            grid_size=5,
            spline_order=3,
            scale_noise=0.1,
            scale_base=1.0,
    ...
    def initialize_centroids(self, X):
        ...
        with torch.no_grad():
            seq_embeddings, _ = self.forward(X_tensor)
        seq_embeddings = seq_embeddings.cpu().numpy()

        indices = np.random.choice(seq_embeddings.shape[0], self.num_cluster, replace=False)
        centroids = seq_embeddings[indices]

        self.prototype_embeddings.data = torch.tensor(
...
class TAHM(nn.Module):
    def __init__(self, args):
        super(TAHM, self).__init__()
        ...
        self.time_interval_embedding = nn.Embedding(
            args.time_interval_max,
            args.hidden_size
        )
        self.a = args.time_interval_a
        self.c = args.time_interval_c
    ...
    def forward(self, input_ids, time_ids):
        ...
        # 时间间隔嵌入
        time_intervals = self._calculate_time_intervals(time_ids)
        time_interval_emb = self.time_interval_embedding(time_intervals)

        # 合并嵌入
        sequence_emb += time_interval_emb
    ...
    def _calculate_time_intervals(self, time_ids):
        """计算时间间隔并应用对数变换"""
        # 计算相邻时间点间隔 (δ_i = t_i - t_{i-1})
        deltas = torch.zeros_like(time_ids)
        if time_ids.size(1) > 1:
            deltas[:, 1:] = time_ids[:, 1:] - time_ids[:, :-1]

        # 应用公式: pos_i = floor(a * log(δ_i / c + 1))
        transformed = torch.floor(self.a * torch.log(deltas / self.c + 1))
        # 确保在合法范围内 [0, time_interval_max-1]
        return torch.clamp(transformed, 0, self.args.time_interval_max - 1).long()
