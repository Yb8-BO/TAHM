...
def get_user_seqs(args):
    ...
    # 时间信息分桶处理
    all_times = []
    for times in time_seq:
        all_times.extend(times)

    # 改进分桶策略：基于时间分布的分位数分桶
    sorted_times = sorted(all_times)
    bucket_edges = [sorted_times[int(len(sorted_times) * i / args.time_buckets)]
                    for i in range(1, args.time_buckets)]
    ...
    for times in time_seq:
        ...
        for t in times:
            # 找到所属分桶
            bucket = 0
            while bucket < len(bucket_edges) and t > bucket_edges[bucket]:
                bucket += 1
            bucket_times.append(bucket)
        time_bucket_seq.append(bucket_times)

    return user_seq, time_bucket_seq, max_item, valid_rating_matrix, test_rating_matrix, not_aug_users
